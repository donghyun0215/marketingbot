...
- [ ] 초안 생성: 주제 → 코드프레소 톤 초안 (Claude API) + **learned_constraints 주입**
- [x] **Voice Score 0~100** (어휘중첩·문장길이분포·용어일관성·구조패턴) + 근거 표시, 실측 분포에서 도출한 임계값 70점 미만은 승인 큐 진입 차단
...
- [ ] **Fact-Guard**: 고객사 실명·가격·계약조건·미검증 수치 스캔 → 하이라이트 + 위험도. 통과 못 하면 승인 요청 자체가 차단

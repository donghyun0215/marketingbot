...
- [x] Telegram Bot: 알림 → 위험 지점 표시 미리보기 → ✅승인/❌반려 버튼 (실전송 확인)
- [x] **반려 사유 → learned_constraints 변환** (Loop 2 폐쇄) — 실동작 확인
- [x] 전 상태 전이 audit_log 기록 + 중복 승인 방지 / [x] 웹 대시보드 승인 큐 (텔레그램 폴백)

### Unit 4 — Data Flywheel  [P1] (친구의 6레이어 기반)
- [x] AI 분석 (레이어 4): 채널별 2σ 이상치 감지, 노출 대비 클릭 0 콘텐츠, 귀속 공백, 검색 수요 공백
- [x] Telegram 데일리 브리핑 (레이어 5) — 실전송 확인
- [x] 주제 추천 (레이어 6) — 채택 시 생성→검사→승인요청까지 자동 연결
- [x] **Loop 1 폐쇄**: 발행 시 tracking_id → `/r/[trackingId]` 추적 링크 → 문의 폼 ref 전달 → `/api/inquiries`에서 content_id 확정 귀속
- [x] **Loop 3**: 승인 발행글 → voice_corpus 자동 편입
- [x] **Loop 4 폐쇄**: 제안→채택→발행→문의 체인을 id로 추적, 발행률·적중률 산출 (집계 저장 없이 매번 체인 계산)

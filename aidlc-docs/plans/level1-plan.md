...
- [~] **반려 사유 → learned_constraints 변환** (Loop 2) — 로직 완성, 버튼 웹훅은 배포 후
- [x] 전 상태 전이 audit_log 기록 + 중복 승인 방지 / [~] 웹 대시보드 승인 큐
...
- [~] 주제 추천 (레이어 6) — 생성·저장 완료, 채택/거절 UI 미구현
- [~] **Loop 1**: 발행 시 추적링크(tracking_id) 생성 완료 / 문의 폼 수신단 미구현
